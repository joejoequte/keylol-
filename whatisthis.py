...
def t683531(appid):
    ...
    if data['release_date']['coming_soon']:
        release_date = f"将于{date}上线"
        summary = f"{genres}游戏《{name}》于{today}在Steam商店上架，计划于{date}上线"
    else:
        if not is_free:
            release_date = f"已于{date}发售"
            final_formatted = data['price_overview'][
                'final_formatted'].replace(' ', '')
    ...
    summary += '\n[color=#ffffff]<以文字简单介绍一些是否支持中文和从游戏官网等处看到的开发进度等Steam商店页没包括的信息。>[/color]\n\n'
    title = f"{genres}游戏《{name}》{release_date}\n\n{summary}"
    print(title)
    ...
    about_the_game = parser.feed(data['about_the_game'])
    p = re.compile('\[size=167\](.*?\[\/size\])')
    about_the_game = p.sub(r'\n[size=4]\1\n', about_the_game)
    print(
        f"[k1]游戏介绍[/k1][color=#ffffff]<这要都是英语，不得翻译翻译？>[/color]\n[quote]{about_the_game}[/quote]"
    )
    ...
    print(
        f"[k1]视频图片[/k1][color=#ffffff]<挑有代表性的放，不用全部放上来。>[/color]\n{movies}\n{screenshots}"
    )
    ...
    # category
    c_table = '[table=200,#101822]\n'
    for tr in soup.select('div#category_block > div'):
        # print(tr.get('class'))
        if tr.get('class')[0] == 'game_area_details_specs':
        ...
        for k, v in keys.items():
            if v[1] != 0:
                ...
                for el in data[k].values():
                    r_table += f"[td][color=#acb2b8]{parser.feed(el)}[/color][/td]"
                r_table += '[/tr]\n'
        r_table += '[/table]'
        print(r_table)
